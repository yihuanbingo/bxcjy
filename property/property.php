...
Property::checkExistHouseholder($community_id,$nav='property');
...
{  
   ...
   $log = Property::getPropertyLog($community_id,$pageNow,$pageNum,$pay_late = strtotime($pay_late),$status,$keywords); 
...
elseif($act=='act_bind')
{
   ...
   {  
	  ...
	  else 
	  {
		 ...
		 $title = current($res);
		 if($Common->charFormat($title[1])!='单元房号' || $Common->charFormat($title[2])!='每月物业费' || $Common->charFormat($title[3])!='物业费缴至年月' || $title[4]!='备注（请不要超过48个字）')
		 { 
		    $msg = array('error'=>1,'message'=>'excel格式须为 单元房号|每月物业费|物业费缴至年月|备注（请不要超过48个字）');
		 }
		 else
		 {
		    $table = $Base->table('log_property');
		    ...
		    foreach($res as $k=>$v)
			{
			   ...
			   {
			      if($Mysql->getOne("select log_id from ".$Base->table('log_property')." where householder_id=$householder_id"))   //更新
			      {
			...
			{
			   $msg = array('error'=>0,'url'=>'/property/property');
			}
			else   //有错误信息，输出提示
			{
			   echo $error_no;
...
elseif($act=='update')
{
   ...
   $sql = "select bp.log_id from ".$Base->table('log_property')." as bp, ".$Base->table('householder')." as bh ".
...
$smarty->assign('nav','property');
chdir('../');
$smarty->display("property/property.htm");
